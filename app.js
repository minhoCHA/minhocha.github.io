...
tl.to(".div-intro-p", { scale: 1, autoAlpha: 0 });
...
var scene_feature_pic = new ScrollMagic.Scene({ 
  ...
  .setTween(".feature-pics-div-img-1", { 
    scale: 1.2
  }) // the tween durtion can be omitted and defaults to 1
  .addIndicators({ name: "feature pic" }) // add indicators (requires plugin)
  .setPin('.feature-pics-div-p-1-1')
  .addTo(controller);

  var scene_feature_pic_p = new ScrollMagic.Scene({
    triggerElement: ".feature-pics-div-img-1",
    triggerHook: 0.9, // show, when scrolled 10% into view
    duration: "80%", // hide 10% before exiting view (80% + 10% from bottom)
    offset: 50 // move trigger to center of element
  })
  .setClassToggle(".feature-pics-div-img-div1", "visible") // add class to reveal
  ...
  $(function () { // wait for document ready
		...
		var controller = new ScrollMagic.Controller({
			globalSceneOptions: {
				triggerHook: '.feature-pics-div-img-div3',
		...
		for (var i=0; i < slides.length; i++) {
			new ScrollMagic.Scene({
				...
				.addIndicators({ name: "slides scene"}) // add indicators (requires plugin)
				.addTo(controller);
		}
  });
  ...
  .setClassToggle(".ffeature-pics-div-img-div4", "visible") // add class to reveal
